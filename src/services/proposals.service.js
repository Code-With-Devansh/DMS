...
import { getAction, resolveCrossTier, assertOnboardRoster } from "./sudoActions.js";
...
export async function fileProposal(actorId, { actionType, payload }, ip) {
  ...
  // Optional DB-aware pre-flight (e.g. ONBOARD_ORG: org exists, roster belongs
  // to it) — fails fast for the proposer instead of only surfacing at execute,
  // possibly after quorum has already been collected. Not a substitute for the
  // same check at execute time: state can still change during the delay window.
  if (action.validateAtFileTime) {
    try {
      await action.validateAtFileTime(payload);
    } catch (err) {
      throw asBadRequest(err);
    }
  }
...
async function onboardOrg(tx, payload) {
  ...
  // Re-validated here even though fileProposal already ran the same guard at
  // file time: the org row or a member's orgId can change during the
  // quorum/delay window, so this — not the file-time check — is the actual
  // enforcement point.
  await assertOnboardRoster(payload);
